...
case class InsertRow(table:Table, values: Tabular) extends PP_SQL_Table{
  def eval: Option[Table] = {
    // list of tables with the values added
    val tables = values.map(value => table.insert(value))
    val bigTable: Tabular = tables.foldRight(List.empty[Row])((elem, acc) => {
      acc ++ elem.data
    })

    // last table will be the version after all updates
    Some(Table(table.name, bigTable))
  }
}
